  ...
  const takePicture = async () => {
    try {
      ...
      if (!res || !res.assets || !res.assets[0]) {
        ...
        console.log(
          '🚀 ~ file: AddCar.tsx ~ line 107 ~ takePicture ~ scanResult',
          scanResult,
        );
        if (!scanResult) {
          //TODO: display error
        ...
        if (scanResult && !scanResult.VIN) {
          //TODO: display message to rescan
          setScanLoading(false);
          return;
        }
      ...
      console.error('error  eto🙋🏼', JSON.stringify(error));
      setScanLoading(false);
      setError(true);
    }
  };
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          {!pictureMode ? (
            <>
              ...
              <View>
                <Text category="label">
                  Numéro d’identification V.I.N. (E.)
                </Text>
                <Controller
                  control={control}
                  rules={{
            ...
            <Step2Picture
              imageUri={pictureUri}
              ocrdata={ocrdata}
              callback={() => navigation.navigate('Home' as any)}
            />
          )}
        </ScrollView>
      </View>
